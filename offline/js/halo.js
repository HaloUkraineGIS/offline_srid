...
var 	FromProjection	= '';//'+proj=longlat +datum=WGS84 +no_defs';
var 	ProjectForSHP	= '';
...
function toSHP()
{
	...
	/*******************************************************************************/
	/* WORK GENERATE POINT WITH NAMES
	/*******************************************************************************
	var Line = CalculateLine(true, true, true);
	var shpwrite = require('js/shpwrite');
	...
	for (i = 0; i < Line.length; i++)
		NewLine[i] = { 
						...
						{ 
							"type": "Point",
							"coordinates": Line[i].point,
						},
						"properties": 
						{
							"PointName" : Line[i].name,
						}
					}
...
/*********************************************************************************/
/*	NOT work FOR polygon
/*********************************************************************************
	
	var write = require('js/src/write'),
    fs = require('js/src/fs');
...
function ClearPoints()
{
	...
	for (i = 0; i < p_markers.length; i++)
		if ("undefined" != typeof(p_markers[i]))
			map.removeLayer(p_markers[i]);
	/**/
...
function GetNameForExport()
{
	if (0 > $("#TaskID").val())
	{
		var Name = $("#TaskIDText").val().trim() + " " + $("#PolygonTypeText").val().trim();
		var TaskID = $("#TaskIDText").val();
		var PolygonType = $("#PolygonTypeText").val();
	}
	else
	{
		var Name = $("#TaskID option:selected").text().trim() + " " + $("#PolygonID").val().trim() + " " + $("#PolygonType option:selected").text().trim();
		var TaskID = $("#TaskID option:selected").text();
		var PolygonType = $("#PolygonType option:selected").text();
	}
